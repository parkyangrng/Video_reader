...
const STRINGS = {
  en: {
    ...
    progressTranslateBatch: 'Translating batch {current} of {total}…',
  ...
  zh: {
    ...
    whisperApiKeyLabel: 'OpenAI API 密钥（用于音频转写）',
    whisperApiKeyHint: '仅用于在本地视频文件暂无字幕时，通过 Whisper 从其音频生成字幕。与上方密钥相互独立——即使摘要使用 Claude，也需要单独填写。可选。',
    cancel: '取消',
    save: '保存',
    ...
    progressConnecting: '正在连接 AI 服务…',
    progressStreaming: '正在接收回复… 已接收 {chars} 个字符',
    progressParsing: '正在解析回复…',
    progressTranslateBatch: '正在翻译第 {current}／{total} 批…',
    errorTimeout: '请求在 {seconds} 秒后超时。字幕可能过长，或网络/接口较慢——请重试，或在"设置"中换用更快的模型。',
    statusManualParsed: '已使用手动粘贴的字幕（{count} 行）。',
    statusNoAutoTranscriptForSource: '此来源无法自动获取字幕。请在下方手动粘贴，或加载 .vtt/.srt 字幕文件。',
    statusNoAutoTranscriptForFile: '该文件无法自动获取字幕。点击"生成摘要与关键点"可自动转写其音频（需要在"设置"中填写 OpenAI 密钥），或在下方手动粘贴 / 加载 .vtt/.srt 字幕文件。',
    audioTranscribeHint: '或自动根据该视频的音频生成字幕（使用 OpenAI Whisper；将在后台静音运行，耗时约为视频时长的四分之一）：',
    audioTranscribeBtn: '根据音频生成字幕',
    transcribingAudio: '转写中…',
    progressRecordingAudio: '正在后台播放并录制音频… {percent}%',
    progressTranscribingChunk: '正在用 Whisper 转写第 {current}／{total} 段…',
    statusAudioTranscribeDone: '已根据音频生成字幕（{count} 行）。',
    statusAudioTranscribeFailed: '根据音频生成字幕失败：{error}',
    statusAudioTranscribeEmpty: '未在该视频音频中检测到语音。',
    statusNeedWhisperKey: '请先在"设置"中填写用于音频转写的 OpenAI 密钥。',
    errorNoAudioSource: '音频转写仅支持已加载的本地视频文件（不支持 YouTube 或远程视频链接）。',
    ...
    errorNeedTranscriptOrWhisperKey: '该视频暂无字幕。请在"设置"中填写 OpenAI 密钥以根据音频自动生成，或在下方粘贴字幕 / 上传字幕文件。',
